...
class ProcessorRunner(BaseRunner):
    ...
    def __init__(self, reset_songs: bool, *args, **kwargs):
        ...
        self._delay = 10
        if self.state.processed_folder is not None and \
    ...
    def loop(self) -> None:
        ...
        if self.state.active_channel_id is None or \
                self.state.live is None or \
                self.state.archive_folder is None:
            return None
    ...
    def _process_cut(self, archives: Dict[str, str], cut: XMMarker,
                     is_song: bool = True) -> bool:
        """ Processes `archives` to splice out an
            instance of `XMMarker` if it exists """

        if self.state.processed_folder is None or \
                self.state.active_channel_id is None or \
                self.state.db is None:
            return False
        ...
        if archive is not None:
            ...
            if is_song:
                ...
                if cut.cut.album is not None and \
                        cut.cut.album.title is not None:
    ...
    def _process_cuts(self, archives: Dict[str, str],
        ...
        for cut in cuts:
            ...
            db_item = None
            if is_song:
                existing = self.state.db.query(Song).filter_by(
                    title=cut.cut.title,
